...
There changes to be made to the `configs.json` file to enable the full version of **erxes** on a **Recommended** server set-up.
...
2. You will now change three settings in the `configs.json` file. The settings that need changing are `ELASTICSEARCH_URL`, `ELK_SYNCER`, and `USE_DASHBOARD`.

### Minimum Server configuration of `configs.json`
...
### Recommended Server configuration of `configs.json`
...
The `configs.json` file is located in the `/home/erxes/erxes.io/erxes/` directory. Navigate to this directory.

```bash
cd /home/erxes/erxes.io/erxes/
```

Next, use the **nano** editor to edit the `configs.json` file. There are other variables in this `configs.json` file, but you will only change `ELASTICSEARCH_URL`, `ELK_SYNCER`, and `USE_DASHBOARD` to match the values as shown below:

```bash
nano configs.json
```

Your `configs.json` should look something like this after you've changed it.
